        ...
        private async void HandleFunctionKeySelectionResult(KeyValue singleKeyValue)
        {
            ...
            switch (singleKeyValue.FunctionKey.Value)
            {
                ...
                case FunctionKeys.MouseMoveAndScrollToBottom:
                    ...
                    string s = "ˈflæbɚˌɡɑːstəd";
                    foreach (char c in s)
                    {
                        if (c.IsAscii())
                        {
                            Console.WriteLine($"'{c}' is an ASCII character.");
                        }
                        else
                        {
                            Console.WriteLine($"'{c}' is a Unicode character.");
                        }
                    }
                    Console.WriteLine("---");
                    string s2 = "ˈflæbɚˌɡæstɪd";
                    foreach (char c in s)
                    {
                        if (c.IsAscii())
                        {
                    ...
                    //str += "        <phoneme alphabet=\"ipa\" ph=\"oʊ\"> tomato </phoneme>";
                    //str += "        <phoneme alphabet=\"ipa\" ph=\"flæbɚˌɡæstɪd\"> flabbergasted </phoneme>";
                    str += "        <phoneme alphabet = \"ipa\" ph = \"fɑːrt\"> flabbergasted </phoneme>";
                    str += "    </voice>";
                    str += "</speak>";


                    //"<speak version=\"1.0\" xmlns=\"http://www.w3.org/2001/10/synthesis\" xml:lang=\"en-US\">    <voice name=\"en - US - JennyNeural\">        <phoneme alphabet=\"ipa\" ph=\"fɑːrt\"> </phoneme>    </voice></speak>"

                    //str = "<speak>This is spoken with a phoneme: <phoneme alphabet = \"ipa\" ph = \"flæbɚˌɡæstɪd\"> flabbergasted </ phoneme >.</ speak >";
                ...
                case FunctionKeys.Pronounce:
                case FunctionKeys.PronounceSlow:
                    var speechStarted1 = audioService.SpeakNewOrInterruptCurrentSpeech(
                        ...
                        Settings.Default.SpeechVoice,
                        true,
                        singleKeyValue.FunctionKey.Value == FunctionKeys.PronounceSlow);
                ...
                case FunctionKeys.TypeLastPhoneme:
                    ...
                    {
                        KeySelectionResult(new KeyValue(lastPhonemePronounced), new List<string>());
                    }
                    break;
        ...
        private async Task CommandList(KeyValue singleKeyValue, List<string> multiKeySelection, List<KeyCommand> commandList, int nestLevel)
        {
            ...
            foreach(KeyCommand keyCommand in commandList)
            {
                ...
                else
                {
                    ...
                    else if (keyCommand.Name == KeyCommands.Say)
                    {
                    ...
                    else if (keyCommand.Name == KeyCommands.Pronounce ||
                             keyCommand.Name == KeyCommands.PronounceSlow )
                    {
                        lastPhonemePronounced = keyCommand.Value;
                        audioService.SpeakNewOrInterruptCurrentSpeech(
                        keyCommand.Value,
                        () => { KeyStateService.KeyDownStates[KeyValues.SpeakKey].Value = KeyDownStates.Up; },
                        Settings.Default.SpeechVolume,
                    ...
                    {
                        // hack: this should probably be separate logic
                        // it currently relies on use having turned off auto whitespace and auto capitals etc
                        Log.InfoFormat("CommandList: Text of [{0}]", keyCommand.Value);
                        KeySelectionResult(new KeyValue(keyCommand.Value), multiKeySelection);
                    }
